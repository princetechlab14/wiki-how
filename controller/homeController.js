...
const Single = async (req, res) => {
    ...
    try {
        ...
        const gameCategories = await GameModel.findAll({ where: whereCondition });
        const sidebarCategories = await categoryList();
        res.render('frontend/single', { game, gameCategories, search, sidebarCategories });
    } catch (error) {
...
const PlayGame = async (req, res) => {
    ...
    try {
        const game = await GameModel.findOne({ where: { slug, status: "Active" } });
        if (!game) return res.send("Game not found");
        console.log("game play=>", game.title);
...
module.exports = { Home, Single, PlayGame, About, PrivacyPolicy, TermUse, ContactUs, ContactUsStore, CategoryGames };
